...
class EarlyStopping:
    ...
    def __call__(self, val_f1_acc, model):

        score = val_f1_acc

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_f1_acc, model)
    ...
    def save_checkpoint(self, val_f1_acc, model):
        if self.verbose:
            print(f'Validation f1 score increased ({self.val_f1_acc:.6f} --> {val_f1_acc:.6f}).  Saving model ...')
            torch.save(model.state_dict(), self.path)
        self.val_f1_acc = val_f1_acc
...
def cal_age_label(label):
    mask_label = (label // 6) % 3
    gender_label = (label // 3) % 2
    age_label = label % 3
    return age_label
...
def train(data_dir, model_dir, args):
    ...
    gender_model_dir = os.path.join(args.gender_dir)
    mask_model_dir = os.path.join(args.mask_dir)
    ...
    if args.criterion == 'f1' or args.criterion == 'label_smoothing' or args.criterion == 'f1cross':
        criterion = create_criterion(args.criterion, classes = 3)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            age_outs = model(inputs)
            age_labels = cal_age_label(labels)
            ...
            if args.mixup and (idx + epoch) % 2:
                inputs, labels_a, labels_b, lam = mixup_data(inputs, age_labels, alpha = 1.0)
                inputs, labels_a, labels_b = map(Variable, (inputs, labels_a, labels_b))

                loss = mixup_criterion(criterion, age_outs, labels_a, labels_b, lam)

                _, predicted = torch.max(age_outs.data, 1)

                #correct = lam * cal_label(gender_preds, mask_preds, age_preds)predicted.eq(labels_a.data).cpu().sum().float()
                #(1 - lam) * predicted.eq(labels_b.data).cpu().sum().float())
                #acc = correct / len(labels)
            
            else:
                preds = cal_label(gender_preds, mask_preds, age_preds)
                loss = criterion(age_outs, age_labels)
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                logger.add_scalar("Train/f1_score", train_f1_acc, epoch * len(train_loader) + idx)
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                if figure is None:
                    if epoch % 2:
                        ...
                        figure = log_confusion_matrix(val_labels_items, val_preds_items, args.num_classes)
            ...
            if val_f1_acc > best_val_f1_acc:
                print(f"New best model for val f1 score : {val_f1_acc:4.2%}! saving the best model..")
                torch.save(model.module.state_dict(), f"{save_dir}/best.pth")
                best_val_f1_acc = val_f1_acc
            torch.save(model.module.state_dict(), f"{save_dir}/last.pth")
            print(
                f"[Val] acc : {val_acc.avg:4.2%}, loss : {val_loss.avg:.4f} || val_f1_acc : {val_f1_acc:.4f} || "
                f"best acc : {best_val_acc:4.2%}, best loss : {best_val_loss:.4f} || best f1 acc : {best_val_f1_acc:.4f}"
            )
            logger.add_scalar("Val/loss", val_loss.avg, epoch)
            logger.add_scalar("Val/accuracy", val_acc.avg, epoch)
            logger.add_scalar("Val/f1_score", val_f1_acc, epoch * len(train_loader) + idx)
            logger.add_figure("results", figure, epoch)
            # logger.add_figure("results1", figure2, epoch)
            
            early_stopping(val_f1_acc, model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gender_dir', type=str, default='model/gender/BasicEpoch10SplitAge58/best.pth', help='gender model dir (default: gender/GenderBasicEpoch10Split/best.pth)')
    parser.add_argument('--mask_dir', type=str, default='model/mask/BasicEpoch10SplitAge58/best.pth', help='mask model dir (default: mask/MaskBasicEpoch10Split/best.pth)')
